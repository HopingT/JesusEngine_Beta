	...
	std::vector<D3D11_INPUT_ELEMENT_DESC> Layout)
{
	if (device.m_device == nullptr)
	{
		...
		WARNING("ERROR: ShaderProgram::init : Error in data from params [CHECK FOR std::vector<D3D11_INPUT_ELEMENT_DESC> Layout]\n");
		exit(1);
	}
...
void ShaderProgram::CreateVertexShader(Device device)
{
	...
	if (FAILED(hr))
	{
		...
		WARNING("ERROR: ShaderProgram::CreateVertexShader : Error in data from method [CHECK FOR CreateVertexShader()]\n");
		exit(1);
	}
	...
	if (FAILED(hr))
	{
		...
		WARNING("ERROR: ShaderProgram::CreateVertexShader : Error in data from method [CHECK FOR device]\n");
		exit(1);
	}
}
...
void ShaderProgram::CreatePixelShader(Device device)
{
	...
	{
		MessageBox(nullptr, "The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", "Error", MB_OK);
		WARNING("ERROR: ShaderProgram::CreateVertexShader : Error in data from method [CHECK FOR CompileShaderFromFile()]\n");
		exit(1);
	}
